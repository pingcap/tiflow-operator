...
func (mcm *MasterConditionManager) Update(ctx context.Context) error {
	...
	mcm.cluster.Status.Master.Synced = false

	ns := mcm.cluster.GetNamespace()
	tcName := mcm.cluster.GetName()

	sts, err := mcm.clientSet.AppsV1().StatefulSets(ns).
		Get(ctx, controller.TiflowMasterMemberName(tcName), metav1.GetOptions{})
	if err != nil {
	...
	upgrading, err := mcm.statefulSetIsUpgrading(ctx, sts)
	if err != nil {
		return err
	}
	// todo: remove this logic to status pkg
	if mcm.cluster.MasterStsDesiredReplicas() != *sts.Spec.Replicas {
		if mcm.cluster.MasterStsDesiredReplicas() > *sts.Spec.Replicas {
			mcm.cluster.Status.Master.Phase = v1alpha1.MasterScalingOut
		}
		mcm.cluster.Status.Master.Phase = v1alpha1.MasterScalingIn
	} else if upgrading {
		mcm.cluster.Status.Master.Phase = v1alpha1.MasterUpgrading
	} else {
		mcm.cluster.Status.Master.Phase = v1alpha1.MasterRunning
	}
	...
	if err != nil {
		...
		if epErr != nil {
			return fmt.Errorf("master [%s/%s] codition failed to get endpoints %s , err: %s, epErr %s",
				ns, tcName, controller.TiflowMasterMemberName(tcName), err, epErr)
		}
...
func (mcm *MasterConditionManager) Check(ctx context.Context) error {
	if mcm.cluster.Heterogeneous() {
		...
		SetTrue(v1alpha1.MasterSynced, &mcm.cluster.Status, metav1.Now())
		return nil
	}
...
func (mcm *MasterConditionManager) updateMembersInfo(mastersInfo tiflowapi.MastersInfo) error {
	...
	for _, m := range mastersInfo.Masters {
		...
		masterName, err := formatMasterName(m.Address)
		if err != nil {
			return err
		}
